...
| `@litelens/design-system/hooks`      | `useCopyToClipboard`, `useMediaQuery`, `useBreakpoint` (+ `BREAKPOINTS` constant)                                                                                                                                                                                                                   |
...
- For layout, always prefer Tailwind's responsive prefixes (`md:hidden`, etc.) over JS-driven show/hide.
- When component _state_ also needs to react to a breakpoint crossing (e.g. auto-closing a mobile-only `Sheet` when the viewport grows into the desktop layout), use `useBreakpoint` from `@litelens/design-system/hooks` instead of a hand-rolled `window.matchMedia` listener — it matches the same `sm`/`md`/`lg`/`xl`/`2xl` scale as the Tailwind utilities via its `BREAKPOINTS` map. See `src/layouts/header/HeaderMobileView.tsx` for the pattern: `const isDesktop = useBreakpoint("md")`, then close state in a `useEffect` keyed on `isDesktop`.
